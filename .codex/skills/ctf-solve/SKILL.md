...
3. Use the returned `solve_launch_context` as the authoritative launch context for this selected challenge. It contains only the current identity, problem information, input fingerprint, declared scope, bounded priority files, observation hints, and environment. Do not load a whole-contest index or Board and do not require the user to run another command.
