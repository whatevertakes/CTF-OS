...
Validate the result through the `CTF-OS-main` bridge into the separate `CTF-OS-claude` runtime without trusting it automatically, perform the cheapest decisive verification, and return control to the existing Solve path. Never submit a flag automatically.
