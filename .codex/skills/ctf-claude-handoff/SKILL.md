...
3. Read only the current run's `RUN_MANIFEST.json`, challenge `SESSION-INPUT.json` or equivalent metadata, typed milestone receipts, actual command receipts, bounded output/evidence, working-PoC verification, and explicitly `REFUTED` or `KILL` experiments. Current-session observations may be used only when they are results of commands that actually ran.
4. Exclude other challenges, generic narrative events, unsupported candidates, confidence-only claims, model/race/scheduler/benchmark state, internal monologue, unexecuted commands, untested ideas, unverified flags, long logs, full transcripts, and internal absolute paths. A receipt named `PRIMITIVE_CONFIRMED` is insufficient without actual evidence, a command result, or a control receipt.
