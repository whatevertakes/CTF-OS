...
def expected_build_sha256(repo_root: Path | None = None) -> str:
    ...
    # Mirror .dockerignore (`**/__pycache__`): host-generated bytecode never
    # enters the build context, so it must not perturb the build identity hash.
    # Otherwise a stray sandbox/**/__pycache__/*.pyc created on the host between
    # `build-images.sh` and `doctor` would make every freshly built image read as
    # "not built from the current sandbox build inputs".
    paths = [
        path
