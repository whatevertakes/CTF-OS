...
def _race_prepare(repo: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        targets = () if service.get("status") == "READY" else _resolve_declared_targets(
            challenge.remotes, docker=args.docker
        )
...
def _race_endgame(repo: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    targets = () if service_network else _resolve_declared_targets(remotes, docker=args.docker)
    lane_service: dict[str, Any] | None = None
    metadata: dict[str, Any] | None = None
    try:
