...
_WORKERS = frozenset({"terra_high", "luna_medium", "sol_high"})
_MODES = frozenset({"direct", "parallel", "escalate"})
_REPRESENTATIONS = frozenset({"state", "protocol", "validation", "algebra", "file-flow"})
_TOP_KEYS = frozenset({
    "solve_target", "representation", "mode", "contracts", "replan_when", "escalate_when",
})
...
class SolvePlanParser:
    ...
    def parse(self, output: str) -> SolvePlan:
        ...
        if not isinstance(contracts_raw, list) or not 1 <= len(contracts_raw) <= 3:
            raise PlanParseError("plan.contracts must contain 1 to 3 contracts")
...
Create at most 3 independent execution contracts. A contract must have a
...
Choose workers: terra_high implements one concrete hypothesis; luna_medium
answers one narrow branching question; sol_high is only for a hard conceptual
fork, contradictory outputs, or after two distinct contracts fail.

Return exactly one JSON object with these exact keys and no others:
{{"solve_target":"...","representation":"state|protocol|validation|algebra|file-flow","mode":"direct|parallel|escalate","contracts":[{{"id":"A","worker":"terra_high|luna_medium|sol_high","exclusive_scope":"...","objective":"...","first_decisive_action":"...","success_condition":"...","stop_condition":"...","handoff":"..."}}],"replan_when":"new decisive result|two contracts terminate|contradiction","escalate_when":"two distinct branches fail or conceptual ambiguity remains"}}"""
