...
def _finish_machine_solve(payload: dict[str, object]) -> None:
    path = Path(payload["run_root"]) / "SWARM.json"
...
def test_corrupt_sibling_blocks_only_when_it_is_selected(repo: Path) -> None:
    ...
    assert json.loads((a_run / "STATE.json").read_text())["status"] == "SWARM_ACTIVE"


def test_parallel_prepare_allows_only_one_machine_active_challenge(repo: Path) -> None:
    _write_two_challenges(repo)
    commands = []
    ...
    assert sorted(code for _stdout, _stderr, code in completed) == [0, 2]
    success = next(json.loads(stdout)["result"] for stdout, _stderr, code in completed if code == 0)
    failure = next(json.loads(stdout) for stdout, _stderr, code in completed if code == 2)
    assert "already owns this machine" in failure["error"]
    assert Path(success["run_root"]).joinpath("SWARM.json").is_file()
    active = []
    for path in (repo / "output").glob("**/runs/*/SWARM.json"):
        if json.loads(path.read_text())["status"] == "ACTIVE":
            active.append(path)
    assert len(active) == 1
    assert not list((repo / "output").glob("*/intake.json"))


def test_active_native_child_blocks_same_challenge_input_refresh(repo: Path) -> None:
    _write_two_challenges(repo)
    prepared = _result(_prepare(repo, "misc/A"))
    packet_result = subprocess.run(
        [
            "python", "-m", "ctf_os.agent_tools", "--repo", str(repo),
            "worker-spawn-packet", "misc/A", "--contest", "Isolation CTF",
            "--model-profile", "terra-high", "--role", "builder",
            "--context-mode", "directed", "--task", "build the current path",
        ],
        capture_output=True, text=True,
    )
    assert packet_result.returncode == 0, packet_result.stdout
    lane = json.loads(packet_result.stdout)["result"]["lane_id"]
    confirmed = subprocess.run(
        [
            "python", "-m", "ctf_os.agent_tools", "--repo", str(repo),
            "worker-spawn-confirm", "misc/A", "--contest", "Isolation CTF",
            "--lane", lane, "--native-session", "thread-builder",
        ],
        capture_output=True, text=True,
    )
    assert confirmed.returncode == 0, confirmed.stdout
    preflight = Path(prepared["preflight_record_path"])
    before = preflight.read_bytes()
    prepared_input = Path(prepared["solve_root"]) / "input" / "a.txt"
    (repo / "incoming" / "Isolation CTF" / "misc" / "A" / "a.txt").write_text(
        "changed while live", encoding="utf-8",
    )

    blocked = _prepare(repo, "misc/A")

    assert blocked.returncode == 2
    assert "already owns this machine" in json.loads(blocked.stdout)["error"]
    assert preflight.read_bytes() == before
    assert prepared_input.read_text() == "alpha"
...
""",
    ...
    _finish_machine_solve(b)
    a_again = _result(_prepare(repo, "misc/A"))
    _finish_machine_solve(a_again)
...
def test_selected_source_change_invalidates_only_selected_workspace(repo: Path) -> None:
    ...
    for relative, content in {
        ...
        "workers/preserved-worker/artifacts/probe.txt": "preserve\n",
    }.items():
        path = b_run / relative
    ...
    assert state_after["status"] == "SWARM_ACTIVE" and state_after["flag_candidate"] is None
