...
- Optional Foundry, forge, cast, anvil, chisel, solc, slither, and halmos
  references; avoid vendoring Echidna.
reference_digest:
...
- Use solc or chisel only for local compile/REPL checks tied to provided source
  or bytecode.
- Use halmos for bounded symbolic checking only after source and invariants are
  clear enough to make the result interpretable.
...
- `halmos` when source-backed symbolic checks are justified.
