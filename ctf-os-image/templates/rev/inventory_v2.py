#!/usr/bin/env python3
"""Produce one bounded, source-bound Rev inventory v2 observation.
...
DOCUMENT_TRANSPORT = "atomic-work-file-plus-exact-stdout-once-v2"
...
_PROFILE_BINTYPES = frozenset(
    {"dex", "elf", "java", "mach_o", "pe", "unknown", "wasm"}
)
_PROFILE_ENDIANS = frozenset({"big", "little", "mixed", "unknown"})
_PROFILE_BITS = frozenset({8, 16, 32, 64})
_RABIN2_INFO_REQUIRED_FIELDS = frozenset(
    {"arch", "binsz", "bintype", "bits", "endian", "havecode"}
)
_BINTYPE_ALIASES = {
    "dex": "dex",
    "elf": "elf",
...
_PRODUCER_ERROR_REASON_MAP = {
    code: f"producer_{code}" for code in _ERROR_CODES
}
_PRE_SOURCE_ERROR_REASONS = frozenset(
    {
        "artifact_too_large",
        "contract_mismatch",
        "duplicate_json_key",
...
_SOURCE_MISMATCH_REASONS = frozenset(
...
_CONTRACT_DESCRIPTOR = {
    ...
    "inventory_max_json_depth": MAX_JSON_DEPTH,
    ...
    "producer_normalization": {
        ...
        "bintype": {
            "aliases": dict(sorted(_BINTYPE_ALIASES.items())),
            "empty": "unknown",
            "input_type": "exact-string",
            "normalization": "strip-lower",
        ...
        "bits": {
            "allowed_nonzero": sorted(_PROFILE_BITS),
            "input_type": "exact-integer",
            "zero": None,
        },
        "endian": {
            "allowed": sorted(_PROFILE_ENDIANS),
            "empty": "unknown",
            "input_type": "exact-string",
            "normalization": "strip-lower",
            "normalized_nonempty_pattern": "[a-z0-9_.+-]{1,64}",
        ...
        "rabin2_info_required_fields": sorted(
            _RABIN2_INFO_REQUIRED_FIELDS
        ),
    ...
    "result_semantics": {
        ...
        "bits": sorted(_PROFILE_BITS),
        "bintypes": sorted(_PROFILE_BINTYPES),
        "endians": sorted(_PROFILE_ENDIANS),
        "error_reason_rules": [
            {
                "condition": condition,
        ...
        "pre_source_error_reasons": sorted(
            _PRE_SOURCE_ERROR_REASONS
        ),
        "producer_error_reason_map": dict(
            sorted(_PRODUCER_ERROR_REASON_MAP.items())
        ),
        "result_shape": (
        ...
        "source_mismatch_reasons": sorted(
            _SOURCE_MISMATCH_REASONS
        ),
        "supported_arch_bits": {
            arch: sorted(bits)
            for arch, bits in sorted(_SUPPORTED_ARCH_BITS.items())
        },
        "verdict_reason_rules": [
            ...
            for condition, verdict, reason_code in _VERDICT_REASON_RULES
        ],
    },
    "source_hash": "sha256",
...
def _normalize_rabin2_info(payload: bytes, source_size: int) -> dict[str, object]:
    try:
        ...
        if not _RABIN2_INFO_REQUIRED_FIELDS <= set(info):
        ...
        endian = (
            raw_endian
            if raw_endian in _PROFILE_ENDIANS
            else "unknown"
        )
        return {
            "arch": arch,
...
def main(argv: list[str]) -> int:
    if len(argv) != 1:
        print(
            "Usage: inventory_v2.py /challenge/BINARY",
            file=sys.stderr,
        )
        return 2
    try:
