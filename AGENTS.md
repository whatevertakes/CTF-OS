...
For “클로드 구조대 준비해라”, “Claude 구조대 준비해라”, “클로드에게 넘길 내용 정리해라”, or “Claude handoff 만들어라”, load `.codex/skills/ctf-claude-handoff/SKILL.md`. This is a Solve termination command: stop new attack experiments immediately, compact only evidence-backed facts and executed solve history into the single repository-local `rescue/<contest>/<challenge>/HANDOFF.md`, then end that challenge solve. Never inspect or move the original ZIP, call a Claude system, or create a Claude workspace, sandbox, job, or start command.
...
- A Claude handoff request takes priority over every new exploit, remote, or recon action. After `HANDOFF.md` is saved and verified, Sol performs no more reasoning or tool execution for that challenge and ends the turn.
