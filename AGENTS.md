...
For every non-trivial solve, Sol records the tier and branch rationale in `DELEGATION_PLAN.json`, reviews the category template, and calls `branch-admit` before native delegation. `delegation-branch-add` records a proposed/admitted branch but does not create a child; `branch-admit` is deterministic decision support, not a launcher. Requested model role/reasoning are intent only. Keep observed runtime fields `null` and `pinning_verified: false` unless the runtime exposes verifiable values. The plan remains available when CLI presentation hides model, reasoning, or prompt details.

Workers may publish compact private checkpoints. Sol alone merges/shows all checkpoints and uses `branch-utility` as advice for continue, cross-pollination, takeover, or termination. Python never enforces a lifecycle decision. Clean-room verification is an overlap exception, but receives only the final artifact, `REPRODUCE.json`, current fingerprint, and expected observable behavior by default—not the attack branch's raw transcript or detailed solution.
...
The canonical sequence is: `prepare-challenge` → compact evidence review → tier decision → `delegation-plan-init` → template review → `branch-admit` → native delegation → branch `RUNNING` record → `sandbox-create` → bounded execution/checkpoints → Sol checkpoint merge and utility review → worker result save/merge → Sol findings → optional clean-room verifier → exploit and `REPRODUCE.json` → Sol-only `replay` → manual submission. `sandbox-create` only creates the execution environment for an already delegated worker.
